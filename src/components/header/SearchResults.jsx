...
const SearchResults = ({
  ...
  return (
    <>
      <div
        style={{}}
        className={`search-results w-full h-96 overflow-y-scroll my-2 bg-zinc-800 absolute z-30 rounded-xl px-2 ${
          resutlsVisible ? "block" : "hidden"
        }`}
      >
        <Link
          href={{
          ...
          results.slice(0, 8).map((result, i) => {
            return (
              <Link href={`/movies/${result.title}`} key={i}>
                <div className="w-full flex my-3 hover:bg-gray-600 p-1 h-24 transition duration-500 rounded-md">
